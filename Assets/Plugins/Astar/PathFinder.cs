...
namespace AStar
{
    public class PathFinder : IFindAPath
    {
        ...
        public Position[] FindPath(Position start, Position end)
        {
            ...
            var pathes = new List<Position[]>();

            while (graph.HasOpenNodes)
            {
                var q = graph.GetOpenNodeWithSmallestF();
                
                pathes.Add(OrderClosedNodesAsArray(graph, q));
                if (q.Position == end)
                {
            ...
            return pathes
                .OrderBy((positions => Difference(positions[^1], end)))
                .ThenBy((positions => positions.Length))
                .ToArray()[0];
            // return new Position[0];
        }

        private int Difference(Position start, Position end)
        {
            int diff = Math.Abs(start.Row - end.Row);
            diff += Math.Abs(start.Column - end.Column);
            return diff;
        }
